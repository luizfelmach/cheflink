...
import { BadgeCheck, CalendarIcon, Loader, Plus } from "lucide-react";
...
import { Company, Order, Service, getCompany } from "@/companies";
...
export default function Company({ params }: { params: { company: string } }) {
  ...
  useEffect(() => {
    ...
    if (!company) {
      ...
      let initialItemState = Array(company.services.length);
      for (var i = 0; i < initialItemState.length; i++)
        initialItemState[i] = Array(0);

      setSelectedItems(initialItemState);
    }
  }, []);
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    selectedServices.forEach((flag, index) => {
      if (flag) {
        let service: Service = {
          name: company!.services[index].name,
          items: [],
        };
    ...
    fetch("/api/order", {
      ...
      .then((data) => {
        if (data != undefined) {
          router.push(`/${params.company}/${data.id}`);
          setLoading(false);
        }
      })
      ...
      {company ? (
        <>
          ...
          >
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{servicesState[selectedIndex].name}</DialogTitle>
                <DialogDescription>
              ...
              <ScrollArea className="h-56">
                <div className="flex flex-wrap justify-center">
                  {servicesState[selectedIndex].items.map((item, index) => {
                    return (
                      <Toggle
                        key={index}
